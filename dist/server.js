...
function parseApiType(url) {
    ...
    if (/\/tv\/\d+/.test(url))
        return 'tv';
    if (/\/person\/\d+/.test(url))
        return 'person';
    if (/\/(popular|top_rated|now_playing|on_the_air|airing_today|trending|discover)/.test(url))
        return 'list';
    return 'other';
}
...
async function proxyImage(imgPath, reply) {
